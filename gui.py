...
class CompressionApp:
    ...
    def select_txt_file(self):
        ...
        if file_path:
            try:
                # Sıkıştırılmış dosya için çıktı yolu belirle
                ...
                if output_path:  # Kullanıcı bir kayıt konumu seçtiyse
                    # Dosya adını al
                    ...
                    # Decompression için yeni bir çıktı yolu belirle
    ...
    def show_text_comparison(self, original_path, decompressed_path, compression_info):
        """Orijinal ve açılmış dosyaları karşılaştıran ekran"""
        ...
        # Sıkıştırma bilgilerini gösteren frame
        ...
        # Dosya içeriklerini gösteren büyük frame
        files_frame = tk.Frame(self.root)
        files_frame.pack(padx=10, pady=10, fill=tk.BOTH, expand=True)
        
        # Orijinal ve açılmış dosya içeriklerini yan yana göster
        ...
        # Orijinal dosya içeriğini göster
        ...
        # Açılmış dosya içeriğini göster
        ...
        # Dosya içeriklerini oku ve göster
        try:
            # Orijinal dosyayı oku
            with open(original_path, 'r') as f:
                original_content = f.read()
                original_text.insert(tk.END, original_content)
                original_text.config(state=tk.DISABLED)  # Salt okunur yap
            
            # Açılmış dosyayı oku
            with open(decompressed_path, 'r') as f:
                decompressed_content = f.read()
                decompressed_text.insert(tk.END, decompressed_content)
                decompressed_text.config(state=tk.DISABLED)  # Salt okunur yap
            
            # Dosyaların eşleşip eşleşmediğini kontrol et
    ...
    def select_bin_file(self):
        ...
        if file_path:
            try:
                # Açılan dosya için çıktı yolu belirle
                output_path = filedialog.asksaveasfilename(
    ...
    def select_image(self, mode, option):
        ...
        if file_path:
            print(f"Selected Image ({mode}, {option}):", file_path)
            messagebox.showinfo("Not Implemented", "Image compression functionality is not fully implemented yet.")
